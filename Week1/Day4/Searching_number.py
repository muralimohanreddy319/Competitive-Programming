...
def contains(ordered_list, number):
    ...
    print ordered_list
    print number
    if(BinarySearch(ordered_list,0,len(ordered_list)-1,number)):
        return True

    return False


def BinarySearch(ordered_list,low,high,number):
    ...
    while(low<=high):
        ...
        if(ordered_list[mid]==number):
            return True
        if(ordered_list[mid]>number):
            return BinarySearch(ordered_list,low,mid-1,number)
        else:
            return BinarySearch(ordered_list,mid+1,high,number)
    return False
